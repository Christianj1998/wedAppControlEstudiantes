...
//funcion que nos permite realizar las consultas para el conteo de alumnos
function getCountAlumnos($conn){
    $query = $conn->prepare("SELECT COUNT(*) AS cantidad FROM alumnos");
    $query ->execute();
    $result = $query->fetch();
    return $result['cantidad'];
}
//funcion que nos permite ralizar la consultas para el conteo de generos de la tabla alumnos
function getCountGenero($conn, $genero){
    $query = $conn->prepare("SELECT COUNT(*) AS cantidad FROM alumnos WHERE genero = :genero");
